...
正式服务端 `v0.4.0` 完整支持 LazyForza `1.4.7`，并与 `1.4.2`–`1.4.6` 的协议和主要比赛流程兼容。增强碰撞证据需要 `1.4.7`；旧客户端不会使用其版本发布后新增的练习项目、进站策略预测、OB 登录、主办方 Logo、赛道文件按需下载和后续维修区路线修正；1.4.2 没有服务端车队下拉框，填写名称能匹配时按名称加入，否则由服务端自动分配空余车队。完整兼容说明见仓库根目录 `README.md`。
