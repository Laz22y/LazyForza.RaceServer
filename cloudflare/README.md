...
正式服务端 `v0.1.1` 完整支持 LazyForza `1.4.4`，并与 `1.4.2`、`1.4.3` 的协议和主要比赛流程兼容。旧客户端不会使用主办方 Logo、赛道文件按需下载和新版维修区路线修正；1.4.2 没有服务端车队下拉框，填写名称能匹配时按名称加入，否则由服务端自动分配空余车队。完整兼容说明见仓库根目录 `README.md`。
...
- 总控可在保存赛道信息后上传对应 `.lfzestate`，Durable Object 托管上限为 1.5 MiB；已有匹配赛道的客户端只读取房间描述，不下载文件，缺少或摘要不一致时才由车手确认是否下载；
